...
@section('title', 'Edit Custom Page - ' . $page->title)
...
<div class="dc-container">
    <div class="dc-welcome-banner">
        <div class="dc-welcome-title">
            ...
            <p>Update title, URL slug, and content for <strong>{{ $page->title }}</strong>.</p>
        </div>
        ...
        <form action="{{ route('admin.pages.update', $page->id) }}" method="POST">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                <div>
                    ...
                    <input type="text" id="pageTitleInput" name="title" class="dc-search-input" style="width: 100%;" value="{{ old('title', $page->title) }}" required>
                </div>
                <div>
                    <label style="display: block; font-weight: 600; font-size: 13px; margin-bottom: 6px;">URL Slug</label>
                    <input type="text" id="pageSlugInput" name="slug" class="dc-search-input" style="width: 100%; font-family: monospace;" value="{{ old('slug', $page->slug) }}">
                </div>
            </div>

            <div style="margin-bottom: 20px;">
                <label style="display: block; font-weight: 600; font-size: 13px; margin-bottom: 6px;">Page Content</label>
                <textarea name="content" class="dc-search-input" style="width: 100%; height: 250px; padding: 14px;">{{ old('content', $page->content) }}</textarea>
            </div>
