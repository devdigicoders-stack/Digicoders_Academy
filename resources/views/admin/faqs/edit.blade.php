...
<div class="dc-container">
    ...
    <div class="dc-welcome-banner">
        <div class="dc-welcome-title">
            <h1>Edit FAQ Item <i class="fa-solid fa-pen-to-square" style="color: var(--dc-green); margin-left: 4px;"></i></h1>
            <p>Update question, answer content, category, and target page assignment.</p>
        </div>
    ...
    <div class="dc-card" style="padding: 24px;">
        <form action="{{ route('admin.faqs.update', $faq->id) }}" method="POST">
            ...
            <div style="margin-bottom: 20px;">
                ...
                <input type="text" name="question" value="{{ old('question', $faq->question) }}" class="dc-search-input" style="width: 100%; font-size: 14px;" required>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                <div>
                    ...
                    <select name="category" class="dc-select-sm" style="width: 100%; height: 38px;" required>
                        ...
                        <option value="{{ $categoryOption }}" {{ old('category', $faq->category) == $categoryOption ? 'selected' : '' }}>{{ $categoryOption }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="page_slug" class="dc-select-sm" style="width: 100%; height: 38px;" required>
                        ...
                        <option value="{{ $slug }}" {{ old('page_slug', $faq->page_slug) == $slug ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label style="display: block; font-weight: 600; font-size: 13px; margin-bottom: 6px;">Sort Position Order</label>
                    <input type="number" name="sort_order" value="{{ old('sort_order', $faq->sort_order) }}" class="dc-search-input" style="width: 100%;">
                </div>
            </div>
            ...
            <div style="margin-bottom: 24px;">
                ...
                <textarea name="answer" rows="5" class="dc-search-input" style="width: 100%; font-family: inherit; line-height: 1.6;" required>{{ old('answer', $faq->answer) }}</textarea>
            </div>

            <!-- Checkbox Toggles -->
            <div style="display: flex; gap: 30px; align-items: center; margin-bottom: 28px; flex-wrap: wrap;">
                <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; user-select: none;">
                    <input type="checkbox" name="status" value="1" {{ old('status', $faq->status) ? 'checked' : '' }} style="width: 18px; height: 18px; accent-color: var(--dc-green);">
