...
@section('content')
    ...
    <div class="dc-container">
        <form id="blogForm" action="{{ route('admin.blogs.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="status" id="blogStatusInput" value="{{ old('status', 'published') }}">

            <div class="dc-welcome-banner">
                <div class="dc-welcome-title">
                    <h1>Rich Blog Editor & SEO Studio ✍️</h1>
                    <p>Create a high-ranking article with Summernote rich editor, auto-slug generator, and draft/publish
                        controls.</p>
                </div>
            ...
            <div style="display: flex; flex-direction: column; gap: 24px;">
                ...
                <div class="dc-card" style="padding: 28px;">
                    <div style="margin-bottom: 20px;">
                        ...
                        <input type="text" id="titleInput" name="title" class="dc-search-input"
                            ...
                            value="{{ old('title') }}"
                            placeholder="e.g. Master Full Stack Web Development in 2026">
                    </div>
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <div
                            ...
                            <input type="text" id="slugInput" name="slug"
                                ...
                                value="{{ old('slug') }}"
                                placeholder="web-development-roadmap-2026">
                        </div>
                    </div>
                    ...
                    <div
                        ...
                        <div>
                            ...
                            <input type="text" name="author" class="dc-search-input"
                                style="width: 100%; background: var(--dc-white);" value="{{ old('author', 'DigiCoders Team') }}">
                        </div>
                            ...
                            <div style="display: flex; flex-wrap: wrap; gap: 8px; background: var(--dc-bg); padding: 12px; border-radius: var(--radius-std); border: 1px solid var(--dc-border);">
                                @foreach($tags as $tag)
                                    <label style="display: inline-flex; align-items: center; gap: 6px; font-size: 12px; font-weight: 600; background: var(--dc-card-bg); color: var(--dc-dark); padding: 5px 12px; border-radius: 20px; border: 1px solid var(--dc-border); cursor: pointer; user-select: none;">
                                        <input type="checkbox" name="tags[]" value="{{ $tag->id }}" {{ is_array(old('tags')) && in_array($tag->id, old('tags')) ? 'checked' : '' }} style="accent-color: var(--dc-green); cursor: pointer;">
                                        <span>#{{ $tag->name }}</span>
                                    </label>
                                @endforeach
                            </div>
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <div id="coverPreviewContainer" style="display: none; margin-bottom: 12px; position: relative; width: 240px; height: 135px; border-radius: 10px; overflow: hidden; border: 2px solid var(--dc-green); background: var(--dc-bg); box-shadow: 0 4px 12px rgba(0, 166, 81, 0.15);">
                            <img id="coverPreviewImage" src="" alt="Cover Live Preview" style="width: 100%; height: 100%; object-fit: cover;">
                            <span style="position: absolute; bottom: 6px; right: 6px; background: var(--dc-green); color: white; padding: 2px 8px; border-radius: 4px; font-size: 10px; font-weight: 700; text-transform: uppercase;">
                                <i class="fa-solid fa-eye" style="margin-right: 3px;"></i> Live Preview
                            </span>
                        </div>

                        <input type="file" name="featured_image" accept="image/*" class="dc-search-input" onchange="previewFeaturedCover(this)">
                        <span style="font-size: 11px; color: var(--dc-light-gray); display: block; margin-top: 4px;">Recommended size: 1200x630px. Selected image will preview instantly above.</span>
                    </div>
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <textarea name="summary" class="dc-search-input" style="width: 100%; height: 70px; padding: 10px;"
                            placeholder="Brief summary of the article for blog card preview...">{{ old('summary') }}</textarea>
                    </div>

                    <div style="margin-bottom: 20px;">
                        <label style="display: block; font-weight: 600; font-size: 13px; margin-bottom: 6px;">Article Body
                            (Summernote WYSIWYG Editor)</label>
                        <textarea id="summernoteEditor" name="content">{!! old('content') !!}</textarea>
                    </div>
                </div>
                ...
                <div class="dc-card" style="padding: 24px; border-left: 4px solid var(--dc-primary); margin-bottom: 24px;">
                    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px; padding-bottom: 12px; border-bottom: 1px solid var(--dc-border);">
                        ...
                        <button type="button" id="btnAddFaq" class="dc-btn dc-btn-outline" style="padding: 6px 14px; font-size: 12px; border-color: var(--dc-primary); color: var(--dc-primary);">
                ...
                <div class="dc-card" style="padding: 28px; border-left: 4px solid var(--dc-green);">
                    ...
                    <div>
                        ...
                        <textarea name="meta_description" class="dc-search-input"
                            ...
                            placeholder="Compelling 150-160 character description for Google Search snippet...">{{ old('meta_description') }}</textarea>
                    </div>
                </div>
            </div>
        </form>
