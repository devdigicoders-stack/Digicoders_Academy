...
<div class="dc-container">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px; margin-bottom: 24px;">
        <div>
            <h1 style="font-size: 20px; font-weight: 600; color: var(--dc-dark); margin-bottom: 4px; display: flex; align-items: center; gap: 10px;">
                <i class="fa-solid fa-envelope-open-text" style="color: var(--dc-green);"></i>
                <span>Contact Enquiries</span>
            </h1>
            <p style="color: var(--dc-light-gray); font-size: 13px; margin: 0;">Manage all contact form submissions from your website visitors.</p>
        </div>
    </div>

    {{-- Stats Summary Badges (Full Width Grid) --}}
    <div style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 14px; margin-bottom: 24px;">
        ...
        <div class="dc-card" style="padding: 18px 20px; display: flex; align-items: center; gap: 14px;">
            <div style="width: 42px; height: 42px; border-radius: 10px; background: rgba(0,166,81,0.1); display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                <i class="fa-solid fa-inbox" style="color: var(--dc-green); font-size: 17px;"></i>
            </div>
            <div>
                <div style="font-size: 22px; font-weight: 800; color: var(--dc-dark); line-height: 1;">{{ $stats['total'] }}</div>
                <div style="font-size: 11.5px; color: var(--dc-light-gray); font-weight: 600; margin-top: 3px;">Total Enquiries</div>
            </div>
        </div>
        {{-- New --}}
        <div class="dc-card" style="padding: 18px 20px; display: flex; align-items: center; gap: 14px;">
            ...
            <div>
                ...
                <div style="font-size: 11.5px; color: var(--dc-light-gray); font-weight: 600; margin-top: 3px;">New</div>
            </div>
        </div>
        {{-- Contacted --}}
        <div class="dc-card" style="padding: 18px 20px; display: flex; align-items: center; gap: 14px;">
    ...
    <div class="dc-card" style="padding: 16px 20px; margin-bottom: 24px;">
        <form action="{{ route('admin.contact-enquiries.index') }}" method="GET" style="display: flex; flex-wrap: wrap; gap: 12px; align-items: flex-end;">
            {{-- Search --}}
            <div style="flex: 1; min-width: 220px; position: relative;">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Name, phone, email or message..."
            ...
            <div style="display: flex; gap: 8px; align-items: flex-end;">
                ...
                <a href="{{ route('admin.contact-enquiries.index') }}" class="dc-btn dc-btn-outline" style="height: 38px; padding: 0 16px; font-size: 13px; font-weight: 600; border-radius: 6px; background: var(--dc-white); text-decoration: none; display: inline-flex; align-items: center;">
    ...
    {{-- Flash messages are shown via the global toast (admin layout auto-handles session('success'/'error')) --}}

    {{-- Bulk Delete Form & Enquiry Table --}}
    <form id="bulkForm" action="{{ route('admin.contact-enquiries.bulkDelete') }}" method="POST">
        @csrf
        ...
        <div style="display: flex; flex-direction: column; gap: 10px; margin-bottom: 24px;">
                ...
                <div style="background: {{ $bg }}; border: 1px solid {{ $borderColor }}; border-radius: 12px; padding: 16px 20px; display: flex; align-items: flex-start; justify-content: space-between; gap: 16px; transition: var(--transition-fast);">
                    ...
                    <div style="display: flex; gap: 14px; align-items: flex-start; flex: 1; min-width: 0;">
                        ...
                        <div style="flex: 1; min-width: 0;">
                            ...
                            <div style="display: flex; gap: 16px; flex-wrap: wrap; font-size: 12.5px; margin-bottom: 8px;">
                                <span style="color: var(--dc-dark-muted); display: flex; align-items: center; gap: 5px;">
                                    <i class="fa-solid fa-phone" style="color: var(--dc-green); font-size: 11px;"></i>
                                    <a href="tel:{{ $item->phone }}" style="color: inherit; text-decoration: none; font-weight: 600;">{{ $item->phone }}</a>
                                </span>
                                @if($item->email)
                                <span style="color: var(--dc-dark-muted); display: flex; align-items: center; gap: 5px;">
                    ...
                    <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 8px; flex-shrink: 0;">
                        {{-- Status Dropdown (Submits external form to avoid form nesting) --}}
                        ...
                        <div style="display: flex; gap: 6px;">
                            ...
                            <button type="button"
                                ...
                                class="dc-action-btn dc-action-delete" title="Delete Enquiry">
                                <i class="fa-solid fa-trash"></i>
                            </button>
                        </div>
                    </div>
            ...
            @empty
                <div class="dc-card" style="padding: 48px 24px; text-align: center;">
                    ...
                    <h3 style="font-size: 16px; font-weight: 700; color: var(--dc-dark); margin-bottom: 6px;">No Enquiries Found</h3>
                    <p style="color: var(--dc-light-gray); font-size: 13px; max-width: 360px; margin: 0 auto 16px auto;">No contact form submissions match your filter criteria.</p>
                    <a href="{{ route('admin.contact-enquiries.index') }}" class="dc-btn dc-btn-outline" style="padding: 8px 16px; font-size: 12px; background: var(--dc-white);">Clear Filters</a>
                </div>
            @endforelse
        </div>
    </form>

    {{-- Hidden Individual Action Forms (Outside bulkForm to avoid HTML form nesting) --}}
    @foreach($enquiries as $item)
        <form id="statusForm-{{ $item->id }}" action="{{ route('admin.contact-enquiries.updateStatus', $item->id) }}" method="POST" style="display: none;">
            @csrf
            <input type="hidden" name="status" id="statusInput-{{ $item->id }}">
        </form>
        <form id="readForm-{{ $item->id }}" action="{{ route('admin.contact-enquiries.markAsRead', $item->id) }}" method="POST" style="display: none;">
            @csrf
        </form>
        <form id="deleteForm-{{ $item->id }}" action="{{ route('admin.contact-enquiries.destroy', $item->id) }}" method="POST" style="display: none;">
            @csrf
            @method('DELETE')
        </form>
    @endforeach
    ...
    @if(method_exists($enquiries, 'hasPages') && $enquiries->hasPages())
        <div class="dc-card" style="padding: 14px 20px; margin-top: 20px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px;">
            <div style="font-size: 12.5px; color: var(--dc-dark-muted); font-weight: 500;">
                Showing <strong>{{ $enquiries->firstItem() ?? 0 }}</strong> to <strong>{{ $enquiries->lastItem() ?? 0 }}</strong> of <strong>{{ $enquiries->total() }}</strong> contact enquiries
            </div>
...
@push('scripts')
<script>
    // Submit Status Form (Triggers external hidden form to avoid HTML form nesting)
    ...
    // Single Enquiry SweetAlert2 Delete Confirmation
    function confirmDeleteEnquiry(id) {
        Swal.fire({
            title: 'Are you sure?',
            text: "You won't be able to revert this enquiry deletion!",
            icon: 'warning',
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        // Bulk Delete SweetAlert2 Confirmation
        if (bulkDeleteBtn && bulkForm) {
            bulkDeleteBtn.addEventListener("click", function (e) {
                ...
                Swal.fire({
                    ...
                    text: `Are you sure you want to delete ${count} selected enquiry(ies)?`,
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#ef4444',
                    cancelButtonColor: '#64748b',
